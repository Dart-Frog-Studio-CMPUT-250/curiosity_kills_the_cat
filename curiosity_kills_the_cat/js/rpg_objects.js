...
Game_System.prototype.playtimeText = function () {
  ...
  return (
    hour.padZero(2) + ":" + min.padZero(2) + ":" + sec.padZero(2)
  );
};
...
Game_Message.prototype.setNumberInput = function (
  variableId,
  maxDigits
) {
...
Game_Message.prototype.setItemChoice = function (
  variableId,
  itemType
) {
...
Game_Screen.prototype.changeWeather = function (
  type,
  power,
  duration
) {
...
Game_Picture.prototype.updateMove = function () {
  if (this._duration > 0) {
    ...
    this._opacity =
      (this._opacity * (d - 1) + this._targetOpacity) / d;
    this._duration--;
  }
};
...
Game_Action.prototype.apply = function (target) {
  ...
  result.missed =
    result.used && Math.random() >= this.itemHit(target);
  result.evaded =
    !result.missed && Math.random() < this.itemEva(target);
...
Game_Action.prototype.applyGuard = function (damage, target) {
  return (
    damage / (damage > 0 && target.isGuard() ? 2 * target.grd : 1)
  );
};
...
Game_Action.prototype.itemEffectRecoverHp = function (
  target,
  effect
) {
  var value =
    (target.mhp * effect.value1 + effect.value2) * target.rec;
  if (this.isItem()) {
...
Game_Action.prototype.itemEffectRecoverMp = function (
  target,
  effect
) {
  var value =
    (target.mmp * effect.value1 + effect.value2) * target.rec;
  if (this.isItem()) {
    value *= this.subject().pha;
  }
  value = Math.floor(value);
...
Game_Action.prototype.itemEffectRemoveState = function (
  target,
  effect
) {
  var chance = effect.value1;
...
Game_Action.prototype.itemEffectAddDebuff = function (
  target,
  effect
) {
  var chance =
...
Game_Action.prototype.itemEffectRemoveBuff = function (
  target,
  effect
) {
...
Game_Action.prototype.itemEffectRemoveDebuff = function (
  target,
  effect
) {
...
Game_Action.prototype.itemEffectLearnSkill = function (
  target,
  effect
) {
  if (target.isActor()) {
...
Game_Action.prototype.itemEffectCommonEvent = function (
  target,
  effect
) {};
...
Game_Action.prototype.lukEffectRate = function (target) {
  return Math.max(
    1.0 + (this.subject().luk - target.luk) * 0.001,
    0.0
  );
};
...
Game_BattlerBase.prototype.resetStateCounts = function (stateId) {
  ...
  this._stateTurns[stateId] =
    state.minTurns + Math.randomInt(variance);
};
...
Game_BattlerBase.prototype.isBuffOrDebuffAffected = function (
  paramId
) {
  return this._buffs[paramId] !== 0;
};
...
Game_BattlerBase.prototype.overwriteBuffTurns = function (
  paramId,
  turns
) {
...
Game_BattlerBase.prototype.buffIconIndex = function (
  buffLevel,
  paramId
) {
  if (buffLevel > 0) {
...
Game_BattlerBase.prototype.meetsUsableItemConditions = function (
  item
) {
...
Game_BattlerBase.prototype.meetsItemConditions = function (item) {
  return (
    this.meetsUsableItemConditions(item) && $gameParty.hasItem(item)
  );
};
...
Game_Battler.prototype.isStateRestrict = function (stateId) {
  return (
    $dataStates[stateId].removeByRestriction && this.isRestricted()
  );
};
...
Game_Battler.prototype.maxSlipDamage = function () {
  return $dataSystem.optSlipDeath
    ? this.hp
...
Game_Actor.prototype.tradeItemWithParty = function (
  newItem,
  oldItem
) {
...
Game_Actor.prototype.attackElements = function () {
  ...
  if (
    this.hasNoWeapons() &&
    !set.contains(this.bareHandsElementId())
  ) {
...
Game_Actor.prototype.finalExpRate = function () {
  return (
    this.exr *
    (this.isBattleMember() ? 1 : this.benchMembersExpRate())
  );
};
...
Game_Actor.prototype.startAnimation = function (
  animationId,
  mirror,
  delay
) {
...
Game_Actor.prototype.maxFloorDamage = function () {
  return $dataSystem.optFloorDeath
    ? this.hp
    : Math.max(this.hp - 1, 0);
};
...
Game_Party.prototype.makeMenuActorPrevious = function () {
  ...
  if (index >= 0) {
    index =
      (index + this.members().length - 1) % this.members().length;
    this.setMenuActor(this.members()[index]);
  } else {
    this.setMenuActor(this.members()[0]);
  }
...
Game_Map.prototype.setupParallax = function () {
  ...
  this._parallaxZero = ImageManager.isZeroParallax(
    this._parallaxName
  );
...
Game_Map.prototype.distance = function (x1, y1, x2, y2) {
  return (
    Math.abs(this.deltaX(x1, x2)) + Math.abs(this.deltaY(y1, y2))
  );
};
...
Game_Map.prototype.isAirshipLandOk = function (x, y) {
  return (
    this.checkPassage(x, y, 0x0800) && this.checkPassage(x, y, 0x0f)
  );
};
...
Game_Map.prototype.isLadder = function (x, y) {
  return (
    this.isValid(x, y) && this.checkLayeredTilesFlags(x, y, 0x20)
  );
};

Game_Map.prototype.isBush = function (x, y) {
  return (
    this.isValid(x, y) && this.checkLayeredTilesFlags(x, y, 0x40)
  );
};

Game_Map.prototype.isCounter = function (x, y) {
  return (
    this.isValid(x, y) && this.checkLayeredTilesFlags(x, y, 0x80)
  );
};

Game_Map.prototype.isDamageFloor = function (x, y) {
  return (
    this.isValid(x, y) && this.checkLayeredTilesFlags(x, y, 0x100)
  );
};
...
Game_CharacterBase.prototype.setMoveFrequency = function (
  moveFrequency
) {
...
Game_CharacterBase.prototype.setPriorityType = function (
  priorityType
) {
...
Game_CharacterBase.prototype.isCollidedWithVehicles = function (
  x,
  y
) {
...
Game_CharacterBase.prototype.updateJump = function () {
  this._jumpCount--;
  this._realX =
    (this._realX * this._jumpCount + this._x) /
    (this._jumpCount + 1.0);
  this._realY =
    (this._realY * this._jumpCount + this._y) /
    (this._jumpCount + 1.0);
  this.refreshBushDepth();
...
Game_CharacterBase.prototype.setImage = function (
  ...
  this._characterName = characterName;
  this._characterIndex = characterIndex;
  this._isObjectCharacter =
    ImageManager.isObjectCharacter(characterName);
};
...
Game_CharacterBase.prototype.checkEventTriggerTouch = function (
  x,
  y
) {
  return false;
...
Game_CharacterBase.prototype.setDirectionFix = function (
  directionFix
) {
...
Game_CharacterBase.prototype.requestAnimation = function (
  animationId
) {
...
Game_Character.prototype = Object.create(
...
Game_Character.prototype.moveAwayFromCharacter = function (
  character
) {
  var sx = this.deltaXFrom(character.x);
  var sy = this.deltaYFrom(character.y);
...
Game_Character.prototype.turnAwayFromCharacter = function (
  character
) {
  var sx = this.deltaXFrom(character.x);
  var sy = this.deltaYFrom(character.y);
...
Game_Character.prototype.advanceMoveRouteIndex = function () {
  ...
  if (
    moveRoute &&
    (this.isMovementSucceeded() || moveRoute.skippable)
  ) {
...
Game_Player.prototype.meetsEncounterConditions = function (
  encounter
) {
  return (
...
Game_Player.prototype.startMapEvent = function (
  x,
  y,
  triggers,
  normal
...
Game_Player.prototype.isOnDamageFloor = function () {
  return (
    $gameMap.isDamageFloor(this.x, this.y) && !this.isInAirship()
  );
};
...
Game_Follower.prototype.refresh = function () {
  var characterName = this.isVisible()
    ? this.actor().characterName()
    : "";
  var characterIndex = this.isVisible()
...
Game_Followers.prototype.reverseEach = function (
  callback,
  thisObject
) {
  this._data.reverse();
...
Game_Event.prototype.isCollidedWithPlayerCharacters = function (
  x,
  y
) {
...
Game_Interpreter.prototype.iterateActorId = function (
  param,
  callback
) {
  if (param === 0) {
...
Game_Interpreter.prototype.iterateActorIndex = function (
  param,
  callback
) {
  if (param < 0) {
...
Game_Interpreter.prototype.iterateEnemyIndex = function (
  param,
  callback
) {
  if (param < 0) {
...
Game_Interpreter.prototype.changeHp = function (
  target,
  value,
  allowDeath
) {
  ...
  switch (this._params[0]) {
    ...
    case 2: // Self Switch
      if (this._eventId > 0) {
        ...
        result =
          $gameSelfSwitches.value(key) === (this._params[2] === 0);
      }
      break;
    case 3: // Timer
...
Game_Interpreter.prototype.gameDataOperand = function (
  type,
  param1,
  param2
) {
