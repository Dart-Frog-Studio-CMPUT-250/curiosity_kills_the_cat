...
(function (_) {
  ...
  SRD.checkRequirements = function () {
    SRD.Requirements.forEach(function (info) {
      ...
      if (info[0] === "plugin") {
        if (
          window.confirm(
            info[1] +
              ...
              " " +
              (info[4]
                ? "v" + String(info[4]) + " or greater "
                : "") +
              "in order to be used. Would you like to open the page for " +
              info[2] +
              "?"
          )
        ) {
        ...
        window.alert(
          info[1] +
            " requires your MV project to be v" +
            info[2] +
            " or greater."
        );
      }
  ...
  _.params = SRD.parse(
    JSON.stringify(PluginManager.parameters("SRD_GameUpgrade")),
    true
  );
  ...
  _.windowSettings["width"] =
    _.params["Screen Resolution"].Width ||
    _.params["Game Resolution"].Width;
  _.windowSettings["height"] =
    _.params["Screen Resolution"].Height ||
    _.params["Game Resolution"].Height;
  _.windowSettings["min_width"] =
    _.params["Minimum Resolution"].Width || null;
  _.windowSettings["min_height"] =
    _.params["Minimum Resolution"].Height || null;
  _.windowSettings["max_width"] =
    _.params["Maximum Resolution"].Width || null;
  _.windowSettings["max_height"] =
    _.params["Maximum Resolution"].Height || null;
  ...
  if (Utils.RPGMAKER_VERSION && Utils.RPGMAKER_VERSION >= "1.3.0") {
    if (PIXI.VERSION > "4.4.0") {
      ...
      if (_.params["Scale Mode"] === "Linear") {
        ...
        PIXI.tilemap.TileRenderer.SCALE_MODE =
  ...
  Graphics._applyGameUpgradeParameters = function () {
    ...
    if (
      !this.isWebGL() &&
      _.params["Garbage Collection Mode"] === "On Scene Change"
    ) {
  ...
  if (typeof ResourceHandler === "function") {
    ResourceHandler._defaultRetryInterval =
      _.params["Retry Intervals"];
  }
  ...
  DataManager.isDatabaseLoaded = function () {
    if (!SRD.DataManager_isDatabaseLoaded.apply(this, arguments))
      return false;
    ...
    if (fs.existsSync(fileLoc)) {
      ...
      if (typeof json["window"] === "object") {
        ...
        for (let i = 0; i < keys.length; i++) {
          if (keys[i] === "height") {
            const offset =
              _.windowSettings["resizable"] === false ? 2 : 0;
            if (
              _.windowSettings["height"] - offset !==
              json["window"]["height"]
            ) {
            ...
            json["window"][keys[i]] !== _.windowSettings[keys[i]]
          ) {
            json["window"] = _.windowSettings;
            hasChanged = true;
            break;
      ...
      if (hasChanged) {
        ...
        alert(
          "Package.json has been updated! Restart the game to see the changes!"
        );
      }
    } else {
      console.log(
  ...
  if (!_.isNewNWjs) {
    ...
    GameWindowManager.isWindowOriginal = function () {
      return (
        Utils.isNwjs() &&
        !require("nw.gui").Window.get()[this._winCode]
      );
    };

    GameWindowManager.createNewWindow = function (gui) {
      return gui.Window.open(
    ...
    GameWindowManager.setupNewWindow = function (window, oldWindow) {
      ...
      this._intendedWindow.on(
        "loaded",
        function () {
          this._intendedWindow.window.SRD.OriginalWindow =
  ...
  _.Game_Interpreter_updateWaitMode =
  ...
  SRD.Game_Interpreter_pluginCommand =
  ...
  if (_.windowSettings["title"]) {
    ...
    _.Scene_Boot_updateDocumentTitle =
  ...
  window.onload = function () {
    if (
      !_.isNewNWjs &&
      _.params["Game Reconstruction (1.5.X & below)"] &&
      GameWindowManager.isWindowOriginal()
    ) {
  ...
  document.body.style["backgroundColor"] =
    _.params["Background Color"];

  document.body.style["image-rendering"] =
    _.params["Image Rendering"];
})(SRD.GameUpgrade);
