...
(() => {
  ...
  const preLoad = String(parameters["preLoad"])
    .split("|")
    .map((id) => Number(id));
  ...
  const QJSE_Game_Interpreter_executeCommand =
  ...
  //
  //=============================================================================
  QJ.SE.spawnEvent = function (bid, mapid, eventId, x, y) {
    //默认为本地图的xy
    if (eventId <= 0) {
  ...
  DataManager.loadSpawnMapData = function (
    ...
    if (Number(mapid) > 0) {
      if (
        ($dataSpawnMapList[mapid] && eventId > 0) ||
        $dataDirectEvent
      ) {
  ...
  //
  //=============================================================================
  Game_Map.prototype.spawnEvent = function (
    bid,
    sourcemapid,
    sourceid,
    x,
  ...
  Spriteset_Map.prototype.clearEvent = function (id) {
    for (var i = 0; i < this._characterSprites.length; i++) {
      ...
      if (event._sourceid != null && id == event._eventId) {
        for (
          var s = 0;
          s < this._characterSprites[i]._animationSprites.length;
          s++
        ) {
          this._tilemap.removeChild(
            this._characterSprites[i]._animationSprites[s]
          );
        }
  ...
  Game_SpawnEvent.prototype.initialize = function (
    ...
    this._sx = x;
    this._sy = y;
    this._event = $dataDirectEvent
      ? $dataDirectEvent
      : $dataSpawnMapList[sourcemapid].events[this._sourceid];
    this._event.x = x;
    this._event.y = y;
